...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):#UI 測試項目請在這修改and Data欄位Item Name也須修改一樣   #UI 治具條件列表請修改Data欄位Cond_Fixture     
    ...
    #已程式完成項目, 讓測項自動預設勾選----------------------------------------------------------------------------------
    OKItem=['PEL_CC準確度','PEL_CC表頭準確度','PEL_CR準確度','PEL_CV準確度','PEL_CV表頭準確度','PEL_CP準確度','CC_BNC_IMON準確度','電壓設定準確度','空戴漏電流確認', '空載漏電流確認', 'IR電壓設定準確度', 'IR電壓調整率', '電壓調整率'
    ...
    def PEL_CC_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                model_config = {
                                'PEL-3021': COMMON_3021,    'LSG-175A':COMMON_3021,  
                                'PEL-3041': COMMON_3041,
                                'PEL-3111': COMMON_3111,  
    ...
    def PEL_CR_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.DUT.setInput_State(1)
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PSW.setOutput_State(1,delay_before=1)
                PCS_CRMEAS=PCS.getReading_DCI(delay_before=0.5)
                DMM_CR_MEAS_V=DMM.getMainReading(delay_before=0.5)
                ...
                FS_TABLE = {
                    ...
                    "PEL-3111"  : COMMON_3111,
                    "PEL-3111H" : COMMON_3111H,
                    "PEL-3031E" : COMMON_3031E,
                    "PEL-3032E" : COMMON_3032E,
                            }
    ...
    def PEL_CV_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PSW.setOutput_State(1,delay_before=1)
                DMM_CV_MEAS=DMM.getMainReading(delay_before=0.5)
    ...
    def PEL_CV_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PSW.setOutput_State(1,delay_before=1)
                DMM_CVMEAS_READBK=DMM.getMainReading(delay_before=0.5)
                PEL_MEAS_READBK=self.DUT.getMeasure_Voltage(delay_before=0.5)
    ...
    def PEL_CP_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                DMM_CP_MEAS_V=DMM.getMainReading(delay_before=0.5)
                PSW_CP_SET_V=PSW.getVoltage()
                PCS_CP_MEAS_A=PCS.getReading_DCI(delay_before=0.5)
                PEL_CP_MEAS=PCS_CP_MEAS_A*DMM_CP_MEAS_V
                COMMON_3021     =   {'fs': 175,  'VAR': 500000}
                COMMON_3041     =   {'fs': 350,   'VAR': 500000}
                COMMON_3111     =   {'fs': 1050,  'VAR': 500000}
                ...
                if any(m in DUT_CP_MODE for m in ('PEL-3041', 'PEL-3021', 'PEL-3111','LSG-175A')):
                    PL= PEL_CP_SET_POWER - ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * fs) - (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)
                    PU= PEL_CP_SET_POWER + ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * fs) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)

                elif any(m in DUT_CP_MODE for m in('PEL-3031AE','PEL-3032AE','PEL-3021H','PEL-3041H','PEL-3111H')):
                    PL= PEL_CP_SET_POWER - ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * fs) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)
                    PU= PEL_CP_SET_POWER + ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * fs) + (DMM_CP_MEAS_V * DMM_CP_MEAS_V / VAR)
    ...
    def PEL_CC_IMON_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PSW.setOutput_State(1,delay_before=1)
                PCS_read=PCS.getReading_DCI(delay_before=0.5)
                PCS_IMO_ACC=PCS.getReading_DCV(delay_before=0.5)
                ...
                MODEL_CONFIG = {
                        'PEL-3021':  {'COMMON': '3021'},'LSG-175A':  {'COMMON': '3021'},
                        'PEL-3041':  {'COMMON': '3041'},
                        'PEL-3111':  {'COMMON': '3111'},
                        'PEL-3021H': {'COMMON': '3021H'},
                        'PEL-3041H': {'COMMON': '3041H'},
                        'PEL-3111H': {'COMMON': '3111H'},
                                }
                COMMON_CONFIG = {
                        '3021': {
                            'FS': {'LOW': 2.857142857, 'MID': 0.028571429, 'HIGH': 0.028571429},
                            'MODE': 'A',
                        },
                        '3041': {
                        ...
                        '3111': {
                            'FS': {'LOW': 0.476190476, 'MID': 0.004761905, 'HIGH': 0.004761905},
                            'MODE': 'A',
                        },
                        '3021H': {
                            'FS': {'LOW': 0.114285714, 'MID': 1.142857143, 'HIGH': 1.142857143},
                            'MODE': 'B',
                        },
                        '3041H': {
                            'FS': {'LOW': 0.057142857, 'MID': 0.571428571, 'HIGH': 0.571428571},
                            'MODE': 'B',
                        },
                        '3111H': {
                            'FS': {'LOW': 19.04761905, 'MID': 0.19047619, 'HIGH': 0.19047619},
                            'MODE': 'B',
                        },
                    }
                TOL_TABLE = {
                ...
                if strIName=='CC_BNC_IMON準確度':
                        r.setValue(TH.col_Measure_Main,PCS_IMO_ACC)
