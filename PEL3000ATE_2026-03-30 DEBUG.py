...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):#UI 測試項目請在這修改and Data欄位Item Name也須修改一樣   #UI 治具條件列表請修改Data欄位Cond_Fixture     
    ...
    #已程式完成項目, 讓測項自動預設勾選----------------------------------------------------------------------------------
    OKItem=['CC Accuracy','PEL_CC表頭準確度','PEL_CR準確度','PEL_CV準確度','PEL_CV表頭準確度','PEL_CP準確度','CC_BNC_IMON準確度','CV_BNC_VMON準確度','電壓設定準確度','空戴漏電流確認', '空載漏電流確認', 'IR電壓設定準確度', 'IR電壓調整率', '電壓調整率'
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                if strIName=='CC Accuracy':
                    ...
                    rComment=f'{{"Measure_1":"Monitor端量測值","Measure_2":"計算實際輸電壓應測得值","DUT設定":{PEL_CC_SET_V}, "DUT讀回":{dut_rdg}, "電壓表讀回":{vm_rdg},"Monitor量測值":{monitor_rdg},"Monitor應測得值":{M_OE},"理想輸出Monitor值":{M_SE}}}'
                    if  r.value(TH.col_Measure_Main)!='':
