...
from PyQt5.QtSql import (QSqlDatabase,  QSqlQueryModel, QSqlQuery, QSqlField)
...
#設定待測機或測試設備的物件-----------------------------------------------------------------------
DUT_RS232=MyInstr.GPT()
DUT_USB=MyInstr.GPT()
DUT_GPIB=MyInstr.GPT()
DMM=MyInstr.DMM()
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def onQApplicationStarted(self):
        try:
            ...
            QMessageBox.critical(self,'onQApplicationStarted',f'Error:{str(e)}')
...
class frmTestRecord(QMdiSubWindow, Ui_frmTRecord):
    ...
    def CellSpan(self):
        ...
        try:
            ...
            print(f"{inspect.currentframe().f_code.co_name} Error: {str(e)}", Qt.red)
        finally:
...
class frmMaintainDB(QMdiSubWindow, Ui_frmMaintainDB):
    ...
    def resizeEvent(self, event): 
        ...
        if not self.isMinimized():        
            self.table1.setGeometry(QtCore.QRect(10, 30, width-170, height-40))
        ...
        try:
            ...
            for r in range(0,rows):
                ...
                for c in range(0, cols):
                    d=str(rsFix[r][c])
                    if d!=NULL and d!=None and d!='None':
                        item=QTableWidgetItem(d)
                        self.table1.setItem(r, c, item)
        except Exception as e:
            print('table_query 錯誤',str(e))
...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):    
    StopTest=False
    BRList=[]       #使用的Baud Rate List
    SkipPort=[]     #使用大寫儲存不檢查的連接埤    
    #已程式完成項目, 讓測項自動預設勾選----------------------------------------------------------------------------------
    OKItem=['電壓設定準確度','空戴漏電流確認', '空載漏電流確認', 'IR電壓設定準確度', 'IR電壓調整率', '電壓調整率'
    ...
    def setDevInfo(self):
        ...
        try:       
            for i in range(0,self.model.rowCount()):            
                ...
                if rdev.value('Name')=='侍測機_RS232':   
                    dev=DUT_RS232                
                elif rdev.value('Name')=='侍測機_USB':
                    # dev=DUT_USB  
                    dev=self.DUT
                elif rdev.value('Name')=='侍測機_GPIB':
                    dev=DUT_GPIB                
    ...
    def onStartClick(self):
        ...
        try:     
            ...
            #------------------------------------------------------------------------------------------------------------
            if fnmatch(self.cboScript.currentText(),'*99*'): #依腳本, 設定DUT物件
                self.DUT=MyInstr.GPT99()
            ...
            if WriteLog:
                ...
                sSql=f"""Insert into TestLog 
                    ...
                    (
                        ...
                        {Meas_M}, {Meas_1}, {Meas_2}, '{rJudge}', '{rComment}', 
                        '{StartDateTime}', '{rdt}'
