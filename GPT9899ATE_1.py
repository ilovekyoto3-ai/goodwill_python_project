#-----------------------------------------------------------------------------------------------------------------------
WTitle="GPT-98_99_10000 ATE測試系統" #視窗標題
appVer='0.1.49'                      #顯示軟體版本
#----------------------------------------------------------------------------------------------------------------------
...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):#UI 測試項目請在這修改and Data欄位Item Name也須修改一樣   #UI 治具條件列表請修改Data欄位Cond_Fixture     
    ...
    #已程式完成項目, 讓測項自動預設勾選----------------------------------------------------------------------------------
    OKItem=['PEL_CC準確度','PEL_CC表頭準確度','電壓設定準確度','空戴漏電流確認', '空載漏電流確認', 'IR電壓設定準確度', 'IR電壓調整率', '電壓調整率'
        ...
        try:
            ...
            if WriteLog:
                ...
                sSql=f"""Insert into TestLog 
                    ...
                    Workstation, Staff, 
                    Measure_Main, LowLimit, UpLimit, Measure_1, Measure_2, Judge, Comment,
                    ...
                    (
                        ...
                        {Meas_M}, {Low_Lim}, {Up_Lim}, {Meas_1}, {Meas_2}, '{rJudge}', '{rComment}', 
                        '{StartDateTime}', '{rdt}'
    ...
    def PEL_CC_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                PSW.setOutput_State(1,delay_before=2)
                PCS_read=PCS.getReading_DCI()
            ...
            self.txtMsg_append(f"{inspect.currentframe().f_code.co_name} Error: \n{str(e)}",Qt.red)
        PSW.setOutput_State(0,delay_after=5)
        self.DUT.CLS()
        self.DUT.setInput_State(0)            #輸出OFF
        self.DUT.closePort()
        PSW.closePort()
    ...
    def PEL_CC_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                MeasDut = "\n".join(MeasDut_List)
                self.txtMsg_append(f"輸出前DUT狀態:\n{str(MeasDut)}")
                self.DUT.setInput_State(1,delay_after=5)
                self.txtMsg_append(f"DUT高壓輸出ON...")
                PSW.setOutput_State(1,delay_before=2.5)
                PCS_CCMEAS_READBK=PCS.getReading_DCI()
                PEL_MEAS_READBK=self.DUT.getMeasure_Current()
                ...
                if strIName=='PEL_CC表頭準確度':
                    r.setValue(TH.col_LowLimit,AL)
                    r.setValue(TH.col_UpLimit,AU)
