...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):#UI 測試項目請在這修改and Data欄位Item Name也須修改一樣   #UI 治具條件列表請修改Data欄位Cond_Fixture     
    ...
    def onStartClick(self):
        ...
        try:     
            ...
            #------------------------------------------------------------------------------------------------------------
            if fnmatch(self.cboScript.currentText(),'*3041*'): #依腳本, 設定DUT物件
    ...
    def PEL_CC_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                FS_IL3041 = 0.7  # 設定 FS.IL3041 的值
                AL= float(PCS_CCMEAS_READBK - ((0.2 / 100) * PCS_CCMEAS_READBK + (0.3 / 100) * FS_IL3041))# 計算下限公式
                print(f"計算結果: {AL}")
                AU= float(PCS_CCMEAS_READBK + ((0.2 / 100) * PCS_CCMEAS_READBK + (0.3 / 100) * FS_IL3041))# 計算上限公式
                print(f"計算結果: {AU}")
    ...
    def PEL_CV_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                FS_VL = 15  # 設定 FS.VL 的值
                AL= float(PCS_CVMEAS_READBK - ((0.1 / 100) * PCS_CVMEAS_READBK + (0.1 / 100) * FS_VL))# 計算下限公式
                print(f"計算結果: {AL}")
                AU= float(PCS_CVMEAS_READBK + ((0.1 / 100) * PCS_CVMEAS_READBK + (0.1 / 100) * FS_VL))# 計算上限公式
                print(f"計算結果: {AU}")
    ...
    def PEL_CP_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                FS_PL3041 = 3.5
                PL= PEL_CP_SET_POWER - ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * FS_PL3041) - (PSW_CP_SET_V * PSW_CP_SET_V / 500000)
                PU= PEL_CP_SET_POWER + ((0.6 / 100) * PEL_CP_SET_POWER + (1.4 / 100) * FS_PL3041) + (PSW_CP_SET_V * PSW_CP_SET_V / 500000)
                print(f"計算結果: {PL}")
                print(f"計算結果: {PU}")
                if strIName=='PEL_CP準確度':
                    r.setValue(TH.col_Measure_Main,PEL_CP_MEAS)
                    r.setValue(TH.col_Deviation_LowLimit,PL)
                    r.setValue(TH.col_Deviation_UpLimit,PU)
                    if  r.value(TH.col_Measure_Main)!='':
                        #self.txtMsg_append(f"DUT Measure=> {MeasDut['Function']}, {MeasDut['Status']}, {str(MeasDut['TVA'])}, {str(MeasDut['TAR'])}, {str(MeasDut['TT'])}")
                        if r.value(TH.col_Measure_Main) > r.value(TH.col_Deviation_LowLimit) and r.value(TH.col_Measure_Main) < r.value(TH.col_Deviation_UpLimit):
                            r.setValue(TH.col_Judge, 'PASS')                    
