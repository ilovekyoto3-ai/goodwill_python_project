...
__author__='Lorenzo'
__description__="PSU ATE測試系統"
__product_name__='ATE_PSU'
...
class MainTest(QtWidgets.QMdiSubWindow, Ui_frmMainTest):#UI 測試項目請在這修改and Data欄位Item Name也須修改一樣   #UI 治具條件列表請修改Data欄位Cond_Fixture     
    ...
    def PEL_CC_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                PCS_CC_MEAS_V=PCS.getReading_DCV(delay_before=1.5)
                PEL_CC_SET_V=self.DUT.getCC_Current()
                model_config = {
                                'PEL-3021':   {'LOW': 0.35, 'MID':35, 'HIGH': 35,  'VAR_1': 500000,  'MODE': 'A'},
                                'PEL-3041':   {'LOW': 0.7, 'MID':70,  'HIGH': 70,  'VAR_1': 500000,  'MODE': 'A'},
                                'PEL-3111':   {'LOW': 2.1, 'MID':210, 'HIGH': 210, 'VAR_1': 500000,  'MODE': 'A'},
                                'PEL-3021H':  {'LOW': 87.5,'MID':8.75,'HIGH': 8.75, 'VAR_1': 3240000, 'VAR_2': 2210000,'MODE': 'B'},
                                'PEL-3041H':  {'LOW': 175, 'MID':17.5, 'HIGH': 17.5,  'VAR_1': 2210000, 'MODE': 'C'},
                                'PEL-3111H':  {'LOW': 0.525,'MID':52.5,'HIGH':52.5,  'VAR_1': 2210000, 'MODE': 'D'},
                                'PEL-3031AE': {'LOW': 60,'MID':60,     'HIGH':60,  'VAR_1': 500000,  'MODE': 'D'},
                                'PEL-3032AE': {'LOW': 15,'MID':15,     'HIGH': 15,  'VAR_1': 500000,  'MODE': 'D'},
                                }
                ...
                if DUT_CP_MODE in model_config:
                    ...
                    if mode == 'A':
                        CL = PEL_CC_SET_V - ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) - (PCS_CC_MEAS_V / VAR_1)
                        CU = PEL_CC_SET_V + ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
                    elif mode == 'B' and oper_mode == 'HIGH':
                        CL = PEL_CC_SET_V - ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
                        CU = PEL_CC_SET_V + ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
                    elif mode == 'B' and oper_mode == 'MID':
                        CL = PEL_CC_SET_V - ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_2)
                        CU = PEL_CC_SET_V + ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_2)
                    elif mode == 'B' and oper_mode == 'LOW':
                        CL = PEL_CC_SET_V - ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1 *1000)
                        CU = PEL_CC_SET_V + ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1 *1000)
                    elif mode == 'C' and oper_mode == 'HIGH':
                        CL = PEL_CC_SET_V - ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
                        CU = PEL_CC_SET_V + ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
                    elif mode == 'C' and oper_mode == 'MID':
                        CL = PEL_CC_SET_V - ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
                        CU = PEL_CC_SET_V + ((0.002 * PEL_CC_SET_V) + (0.001 * fs)) + (PCS_CC_MEAS_V / VAR_1)
    ...
    def PEL_CC_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                model_config = {
                                'PEL-3021':   {'LOW': 0.35, 'MID':35, 'HIGH': 35,   'MODE': 'A'},
                                'PEL-3041':   {'LOW': 0.7, 'MID':70,  'HIGH': 70,   'MODE': 'A'},
                                'PEL-3111':   {'LOW': 2.1, 'MID':210, 'HIGH': 210,  'MODE': 'A'},
                                'PEL-3021H':  {'LOW': 87.5,'MID':8.75,'HIGH': 8.75, 'MODE': 'A'},
                                'PEL-3041H':  {'LOW': 175, 'MID':17.5, 'HIGH': 17.5, 'MODE': 'A'},
                                'PEL-3111H':  {'LOW': 0.525,'MID':52.5,'HIGH':52.5, 'MODE': 'A'},
                                'PEL-3031AE': {'LOW': 60,   'MID':60,     'HIGH':60,   'MODE': 'B'},
                                'PEL-3032AE': {'LOW': 15,   'MID':15,     'HIGH': 15,  'MODE': 'B'},
                                }
    ...
    def PEL_CR_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                PCS_CR_MEAS_V=PCS.getReading_DCV(delay_before=1.5)
                ...
                if model and oper_mode in FS_TABLE[model]:
                    ...
                    if oper_mode == "LOW":
                        if ['PEL-3111H'] in model:
                            PEL_CR_STANDARD = PCS_CR_MEAS_V * PEL_CR_RE / 1000
                            RL = PEL_CR_STANDARD - ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN)
                            RU = PEL_CR_STANDARD + ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN)
                        elif model.endswith("H"):
                            PEL_CR_STANDARD = PCS_CR_MEAS_V * PEL_CR_RE
                            RL = PEL_CR_STANDARD - ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN * 1000)
                            RU = PEL_CR_STANDARD + ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN * 1000)
                            ...
                            RL = PEL_CR_STANDARD - ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) - (PCS_CR_MEAS_V / V_DEN)
                            RU = PEL_CR_STANDARD + ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN)
                    elif ['PEL-3021H','PEL-3041H'] in model:
                        PEL_CR_STANDARD = PCS_CR_MEAS_V * PEL_CR_RE / 1000
                        RL = PEL_CR_STANDARD - ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN)
                        RU = PEL_CR_STANDARD + ((0.005 * PEL_CR_STANDARD) + (0.005 * FS)) + (PCS_CR_MEAS_V / V_DEN)
                    elif model.endswith("E"):
                ...
                if strIName=='PEL_CR準確度':
                    if model.endswith("E"):
                        PCS_CRMEAS_mS = (PCS_CRMEAS / PCS_CR_MEAS_V)*1000
    ...
    def PEL_CV_Accuracy(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PSW.setOutput_State(1,delay_before=1.5)
                PCS_CV_MEAS=PCS.getReading_DCV(delay_before=1.5)
                ...
                if strIName=='PEL_CV準確度':
                    r.setValue(TH.col_Measure_Main,PCS_CV_MEAS)
    ...
    def PEL_CV_Mea_Readback(self,strIName='',FixUse='all'):
        ...
        try:
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                self.txtMsg_append(f"DUT_LOAD_ON...")
                PSW.setOutput_State(1,delay_before=1.5)
                PCS_CVMEAS_READBK=PCS.getReading_DCV(delay_before=1.5)
                ...
                if DUT_CP_MODE in model_config:
                    ...
                    if mode == 'A':
                        CVL = PCS_CVMEAS_READBK - ((0.001 * PCS_CVMEAS_READBK) + (0.001 * fs))
                        CVU = PCS_CVMEAS_READBK + ((0.001 * PCS_CVMEAS_READBK) + (0.001 * fs))
    ...
    #電壓設定準確度
    def PQC_HV_Accuracy(self,strIName='',FixUse='all'):
        TH=recordHeader()
        RampTime=0.5            #預設電壓上升時間
        sJudge=''
        try:
            if self.DUT.instrConnected==0 or  not (HVM.instrConnected==1 or self.chkHVMManual.isChecked()):     #確認此項需要的設備是否有連接
            ...
            QtWidgets.QApplication.processEvents()
            self.DUT.openPort()
            if self.chkHVMManual.isChecked==False: HVM.openPort() #非手動高壓表填值,開啟高壓表
            self.DUT.CLS()
            self.delay(0.1)
            self.DUT.setRampTime(RampTime)#設定上升時間
            self.delay(0.1)
            i2t_l=''
            i3t_l=''
            strM=''
            ...
            for i in range(0,window.TRecord.model.rowCount()):                  #開始掃報告的每一列
                ...
                try:
                    ...
                    rHiSet=''
                rindex = window.TRecord.model.index(i, 0)######更新GUI
                window.TRecord.model.TestRow=i           
                window.TRecord.tbview.scrollTo(rindex)
                ...
                if strM_l!=strM:                    
                    self.DUT.setRampTime(RampTime)              #設定上升時間
                    self.txtMsg_append(f'設定Ramp Time:{RampTime}')                    
                    if strM.find('DC')>=0:
                        self.DUT.setEditMode('DCW',0,0.02)
                        self.DUT.setDCW_LoSet(0)
                        if rHiSet=='':
                            self.DUT.setDCW_HiSet(self.default_DCW_HiSet)
                        else:
                        ...
                        if rHiSet=='':
                            self.DUT.setACW_HiSet(self.default_ACW_HiSet) 
                        else:
                            self.DUT.setACW_HiSet(rHiSet)
                        self.DUT.setACW_Timer(60)
                        if strM.find('AC50HZ')>=0:
                ...
                if strM.find('DC')>=0:
                    ...
                    self.DUT.setACW_Volt(r.value(TH.col_Cond_1))
                
                MeasDut=self.DUT.getMeasure()
                DErr=self.DUT.getSysErr()   #取得系統錯誤訊息.
                self.txtMsg_append(f"輸出前DUT狀態:\n{str(MeasDut)}\nSYS Err:{DErr}")
                self.delay(0.3,stm)                 #確定切換治具後超過0.3秒再輸出.                
                self.DUT.Test(1)
                self.txtMsg_append(f"DUT高壓輸出ON...")
                # self.delay(RampTime+0.5)
                self.delay(RampTime+1)
                MeasDut=self.DUT.getMeasure()
                DErr=self.DUT.getSysErr()   #取得系統錯誤訊息.
                self.txtMsg_append(f"輸出後DUT狀態:\n{str(MeasDut)}\nSYS Err:{DErr}")
                self.txtMsg_append(f"Measure?傳回:\n{self.DUT.getMeasure_RawStr()}")
                rdg_dut=''
                rdg_hvm=''
                ae=r.value(TH.col_UpLimit)-r.value(TH.col_Expect)
                ...
                if self.chkHVMManual.isChecked()==False:
                    ...
                    r.setValue(TH.col_Comment, rComment)
                    j=0
                    while(j<3 and not self.StopTest):  #若不在規格內, 再等一下, 再重讀值
                        MeasDut=self.DUT.getMeasure()                      
                        # if MeasDut['Status']!="ERROR" and MeasDut['Status']!="FAIL":
                        if MeasDut['Status']=="TEST":    
                            self.delay(1.5)                            
                            if strM.find('DC')>=0:
                                rdg_hvm=HVM.getDCV()              
                            else:
                                rdg_hvm=HVM.getACV()
                            if rdg_hvm != '':
                                rdg_hvm=rdg_hvm/1000    #轉成KV
                            # MeasDut=self.DUT.getMeasure()
                            if isnumber(MeasDut['TVA']):
                                rdg_dut=float(MeasDut['TVA'])
                            else:
                                rdg_dut=''
                            self.txtMsg_append(f"{j}次讀值, 高壓表=>{str(rdg_hvm)} kV, DUT=>{str(rdg_dut)}, MeasDut['Status']=>{MeasDut['Status']}")    
                        else:
                            self.txtMsg_append(f"DUT狀態:{MeasDut['Status']},不在'TEST', 判定無效值, 不取值!",Qt.red)
                            DErr=self.DUT.getSysErr()
                            self.txtMsg_append(f"{j}次讀值, 高壓表=>{str(rdg_hvm)} kV, DUT=>{str(rdg_dut)}, MeasDut['Status']=>{MeasDut['Status']}, \nDErr=>{DErr}",Qt.red)
                            break                        
                        if isnumber(rdg_dut) and isnumber(rdg_hvm):
                            if rdg_dut>al and abs(rdg_dut-rdg_hvm)<ae: break
                        j+=1                    
                else:
                    if not self.StopTest:
                        rComment='手填值 '
                ...
                MeasDut=self.DUT.getMeasure()
                if strIName=='電壓量測準確度':
                    rComment+="M1:HVM讀值,M2:DUT讀值"
                    if isnumber(MeasDut['TVA']):
                        rdg_dut=float(MeasDut['TVA'])
                    else:
                        rdg_dut=''
                    if rdg_dut!='' and rdg_hvm!='':
                        rdg=r.value(TH.col_Expect)+(rdg_dut-rdg_hvm)
                        r.setValue(TH.col_Measure_Main,rdg)
                        r.setValue(TH.col_Measure_1,rdg_hvm)
                ...
                if MeasDut['Status']!="ERROR" and r.value(TH.col_Measure_Main)!='':
                    ...
                    rComment+=f",Status: {MeasDut['Status']}"
                r.setValue(TH.col_Comment, rComment)
                self.Update_TRecord(r,True,rStartTime)      #更新資料列
                ...
                self.txtMsg_append(f"DUT高壓輸出OFF.")
                self.delay(0.1)
        except Exception as e:
            self.txtMsg_append(f"{inspect.currentframe().f_code.co_name} Error: \n{str(e)}",Qt.red)
        self.DUT.CLS()
    ...
    #電流量測準確度
    def I_Meas_Accuracy(self,strIName='',FixUse='all'):
        subProcName="I_Meas_Accuracy"
        TH=recordHeader()
        sJudge=''
        ...
        try:      
            for i in range(0,window.TRecord.model.rowCount()):             
                ...
                if self.chkFixOptimize.isChecked()==False and not(r.value(TH.col_Cond_Fixture) in self.checkedFix):
                    continue                #此治具没有勾選, 執行下一回圈
                rStartTime=QDateTime.currentDateTime().toString(self.MyDTFormat)        #記錄此筆測試開始時間
                self.txtMsg_append(f"{r.value(TH.col_ItemName)},{r.value(TH.col_ItemName_2t)},{r.value(TH.col_ItemName_3t)}, {r.value(TH.col_Cond_1)}, {r.value(TH.col_CondUnit_1)}, {r.value(TH.col_Cond_Fixture)} 確認中...")
                self.Blank_TRecord(r)
                rComment=''
                try:
                    cond_DUT=eval(r.value(TH.col_Cond_DUT))
                ...
                if TMode_l!=TMode:
                    ...
                    if TMode=='DC':
                        ...
                        DMM.setRefleshRate('M')
                        DMM.setDCIRESolution('0.001')
                        self.DUT.setEditMode('DCW',0,0.02)
                        self.DUT.setDCW_LoSet(0)
                        #self.DUT.setDCW_HiSet(5)
                        self.DUT.setDCW_Timer(60)                    
                    else:
                        DMM.setACI('0.1') #100mA檔
                        DMM.setRefleshRate('M')
                        DMM.setACIRESolution('0.001')                    
                        self.DUT.setEditMode('ACW',0,0.02)
                        self.DUT.setACW_LoSet(0)
                        #self.DUT.setACW_HiSet(10)  
                        self.DUT.setACW_Timer(60)                    
                        if TMode=='AC50HZ':
                            self.DUT.setACW_Freq(50)
                        elif TMode=='AC60HZ':
                ...
                if TMode=='DC':                
                    self.DUT.setDCW_Volt(r.value(TH.col_Cond_1))
                    if rHiSet=='': rHiSet= self.default_DCW_HiSet    
                    rHiSet=float(rHiSet)
                    if rHiSet>11 and (fnmatch(self.cboScript.currentText(),'*98*') or fnmatch(self.cboScript.currentText(),'*120*')): 
                        self.txtMsg_append(f"HiSet要求設定{rHiSet}過大, 改設為11",Qt.red)                        
                        rHiSet=11 #DC最大設定值為11mA                
                    self.DUT.setDCW_HiSet(rHiSet)
                    DMM.setRefleshRate('M')
                    if rHiSet<1:
                        DMM.setDCI('0.001') #1mA檔                    
                        DMM.setDCIRESolution('0.00001')
                    elif rHiSet<10:
                        DMM.setDCI('0.01') #10mA檔                    
                        DMM.setDCIRESolution('0.0001')
                    else:
                        DMM.setDCI('0.1') #100mA檔                    
                        DMM.setDCIRESolution('0.001')
                else:                
                    self.DUT.setACW_Volt(r.value(TH.col_Cond_1))
                    if rHiSet=='': rHiSet= self.default_ACW_HiSet
                    rHiSet=float(rHiSet)
                    self.DUT.setACW_HiSet(rHiSet)
                    DMM.setRefleshRate('M')
                    if float(rHiSet)<1:
                        DMM.setACI('0.001') #1mA檔                    
                        DMM.setACIRESolution('0.00001')
                    elif float(rHiSet)<10:
                        DMM.setACI('0.01') #10mA檔                    
                        DMM.setACIRESolution('0.0001')
                    else:
                        DMM.setACI('0.1') #100mA檔                    
                        DMM.setACIRESolution('0.001')                    
                self.delay(0.1)
                ...
                # self.delay(RampTime+2)
                # MeasDut=self.DUT.getMeasure()
                V=r.value(TH.col_Cond_1)
                VStep=float(round(V*0.02,3))
                VLowLim=V*0.8
                out_st=monotonic()
                while (monotonic()-out_st)<(RampTime+2):
                    self.delay(0.5)
                    MeasDut=self.DUT.getMeasure()
                    DErr=self.DUT.getSysErr()   #取得系統錯誤訊息.
                    # DErr=str(DErr).strip().replace("'","").replace("[","").replace("]","")
                    if MeasDut['Status']=="ERROR" or DErr!="":                        
                        if str(DErr).lower().find('over')>=0:
                            V-=VStep
                            ...
                            last_v=V   #最後電壓設定值
                            self.txtMsg_append(f"發現錯誤({DErr}), 重設電壓:{V}")
                    if MeasDut['Status'] in ["FAIL","ARC"]: break
                    if self.StopTest: break  

                rdg_mA=''
                rComment=''
                r.setValue(TH.col_Comment, rComment)
                if not(MeasDut['Status'] in ["ERROR","FAIL", "ARC"]):
                    self.delay(2)
                elif MeasDut['Status']=="FAIL":#fail調降電壓
                    MeasDut=self.DUT.getMeasure()
                    self.DUT.Test(0)
                    # V=r.value(TH.col_Cond_1)
                    # VStep=round(V*0.02,3)
                    rv=0
                    k=0
                    hasVLow=False
                    self.txtMsg_append(f"DUT狀態:{MeasDut['Status']}, 電壓:{MeasDut['TVA']}, 電流:{MeasDut['TAR']}, 將自動調降電壓... ",QColor('darkorchid'))
                    while(MeasDut['Status']=="FAIL" and rv<5 and hasVLow==False):     #最多調rv次, 調降電壓值, 避免電流過大, 造成超出HISET的FIAL狀況
                        self.DUT.Test(0)
                        self.delay(0.1)
                        RampTime+=1
                        self.DUT.setRampTime(RampTime)#設定上升時間
                        ...
                        if k==0:
                            ...
                            V=float(round(r.value(TH.col_Expect)*R1,3))
                        else:
                            V=V-VStep
                        if V<VLowLim:
                            V=VLowLim
                        ...
                        self.delay(RampTime+1)
                        MeasDut=self.DUT.getMeasure()
                        self.txtMsg_append(f"自動調降電壓, 設定電壓:{V}, DUT回傳 狀態:{MeasDut['Status']}, 電壓:{MeasDut['TVA']}, 電流:{MeasDut['TAR']}",QColor('darkorchid'))
                        rv+=1
                        k+=1
                    if not MeasDut['Status'] in ["FAIL","ERROR","ARC"]: self.delay(2)
                j=0
                ae=r.value(TH.col_UpLimit)-r.value(TH.col_Expect)
                au=r.value(TH.col_Expect)+ae*10     #暫訂DUT讀值要處範圍為10倍公差內
                al=r.value(TH.col_Expect)-ae*10
                rComment+="M1:DMM讀值,M2:DUT讀值"
                while(j<3):     #讀值至規格內, 最多讀3次 (避免時間不足的讀值錯誤) 
                    rdg_mA=DMM.getMainReading() #DMM讀值
                    if rdg_mA=='':
                        self.txtMsg_append("DMM没有讀回值",Qt.red)                        
                    # DMM.closePort()
                    if rdg_mA != '':
                        rdg_mA=rdg_mA*1000    #轉成mA
                    self.txtMsg_append(f"{j}次, DMM讀值=> {str(rdg_mA)} mA")
                    MeasDut=self.DUT.getMeasure()                    
                    if isnumber(MeasDut['TAR']):
                        rdg_dut=float(MeasDut['TAR'])
                    else:
                        rdg_dut=''
                    if abs(rdg_dut-rdg_mA)<ae and rdg_dut>al: break
                    if MeasDut['Status'] in ["FAIL","ERROR","ARC"]:break
                    self.delay(1)
                    j+=1 
                
                if rdg_dut!='' and rdg_mA!='' and rdg_dut<au and rdg_dut>al and rdg_mA<au and rdg_mA>al:
